...
**Remaining open question (resolve by directed test, not assumption):** (a) do issuance transactions (`AssetManager.IssueAsync` / `IssuanceParams`) flow through the emulator co-sign path the same way spends do — i.e. can NArk's issuance path (see `NArk.Core/Assets/AssetPacketBuilder.cs`, the packet composer) be combined with the EmulatorPacket in one extension, or must the breed tx be hand-assembled the way `CovenantSpender` assembles spends? Probe with a minimal issuance-under-covenant tx (fund a covenant VTXO, spend it in a tx that also carries an issuance asset group + the EmulatorPacket) before designing the full flow.

...
- Route sensitive money-path / concurrency / durability fixes to the `sensitive-bugfix` agent, then verify its "done" yourself before merging. It runs on **Opus** (`model: claude-opus-5` in its own frontmatter). **Fable is OPT-IN and never the default** — the global CLAUDE.md and both agent descriptions say so outright; this line used to claim the reverse ("Fable-pinned; Opus fallback"), which would have sent every money-path fix to a model the user has explicitly opted out of. Same rule for `deep-planner`. The live prioritized queue is the **auto-memory backlog** (`arkade-heroes-backlog.md`), not the harness task list — the task list is now a long historical log.

...
# E2E is dispatch-only on PRs (`gh workflow run CI --ref <branch>`). Every subsystem now has some E2E,
# but it drives HTTP against an ALREADY-BUILT host, so it cannot see a composition-root defect (#278 was
# found by booting the thing, not by this). Name the suite that gated your change, not the one that ran.

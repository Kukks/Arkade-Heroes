...
- **Decision-ready and waiting on the user — these are GitHub issues, not prose here, and each carries its measurements:** **#294** the free on-ramp (trials) returns nothing to 97.8% of recruits and 24/24 real onboarding runs, with a measured fix and the rejected alternative; **#273** four economy/protocol calls (marketplace fee floor, the season pot's house-funded base, #253/BID-SURPLUS, the SDK submodule bump); **#228** a renewal downgrading a live receive address, confirmed reachable, fix lives in the submodule so it is warn-and-ask; **PR #253** still open on the covenant input-value constraint.

- **User-owned decisions — do NOT decide these unilaterally, surface them:** combat balance (measured: ~41% of equal-level matches are pre-decided before the seed — a design call, not a bug. `CombatAttributionProbe` now splits the outcome: **breeding 41.6% / entropy 30.5% / gear 12.3% / level 6.7%**, and a tier-3 set already flips 99.7% of matchups a moderately better-bred hero would win, so the shipped ordering already matches the stated intent — breeding first, gear and level real. What drives the pre-decided rate is resolver SENSITIVITY, not weak gear or level: ONE extra level is a 10.7% edge on `ATK*HP*(DEF+25)` and already a 73.4% win rate. Raising gear/level influence makes pre-decided **worse**, and the algebra forbids buying both from the same 100% — see the probe's class summary); legal/regulatory review (permadeath wagering + real-BTC cash-out — see `arkade-heroes-legal-risk.md`); whether to flip the marketplace listing fee default-on; browser fee-flow verification (#212 — the server side is E2E-proven; the browser click-through needs a human watching the UI).
...
- **The general working rules are NOT restated here. They live in the global `CLAUDE.md`, which is always in context — read them there.** That covers AI attribution, time estimates, the comment cap, never skipping tests, submodule warn-and-ask, `gh` heredocs, staging by explicit path, PR monitoring across all three review surfaces, and model routing. This section used to paraphrase some of them, and the single rule it paraphrased in most detail — agent model routing — is the one that ended up **inverted**, telling the next session to send money-path fixes to a model the user had opted out of. A recap of rules that live elsewhere is a copy that will drift, and this file has now proved that twice in one day (see also the test counts, which disagreed with themselves across three places).
- Repo-specific and therefore genuinely here: commit messages are dense and technical, and capture the WHY and the traps rather than the what — see `git log` for the house style. The user reads results asynchronously, so close each turn with what ran, what is confirmed versus inferred, and the short commit hashes.
- The live prioritized queue is the **auto-memory backlog** (`arkade-heroes-backlog.md`), not the harness task list — the task list is now a long historical log. Decisions waiting on the user are GitHub issues; see §7.

...
- **The wager escrow is live** (`WagerEscrowCovenantTests` + `ArkadeCovenants.{Sha256Gate,AtomicSweep,SettleWithSeed}`): two players stake into one escrow whose settle branches require the pre-committed server seed revealed AND both stakes spent atomically with the full pot to the branch winner. Wrong seed refused; pot siphoning refused; honest settle sweeps both stakes in one co-signed tx. Remaining gap to the `.ark` spec: branch authorization (today either branch is satisfiable once the seed is public — settlement is server-driven, so between the two players this is liveness, not theft). The planned fix converges with receipts: settle branches gain `OP_CHECKSIGFROMSTACK` over the match's signed ProgressionReceipt payload, making the receipt itself the on-chain oracle artifact.

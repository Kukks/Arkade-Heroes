...
public class MergeFlowTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task Merge_ConsumesBothInputs_MintsAFusedHero_InheritsBaseLevel()
    {
        ...
        var reveal = await alice.Merge.RevealAsync(commit.MergeId, new MergeRevealRequest("merge-nonce"));
    ...
    public async Task Merge_RejectsSelf_Unowned_AndRevealBeforeFunding()
    {
        ...
        await Assert.ThrowsAsync<ArkadeHeroesApiException>(
            () => alice.Merge.CommitAsync(new MergeCommitRequest(a[0].Id, a[0].Id)));

        // Merging someone else's hero is refused.
        await Assert.ThrowsAsync<ArkadeHeroesApiException>(
            () => alice.Merge.CommitAsync(new MergeCommitRequest(a[0].Id, b[0].Id)));

        // Reveal before the escrow is funded is refused (deposit-gated).
        var commit = await alice.Merge.CommitAsync(new MergeCommitRequest(a[0].Id, a[1].Id));
        await Assert.ThrowsAsync<ArkadeHeroesApiException>(
    ...
    public async Task Merge_EscrowParamsAreServedForTrustlessRebuild()
    {
        ...
        var escrow = await alice.Merge.EscrowAsync(commit.MergeId);
        Assert.Equal(commit.MergeId, escrow.MergeId);
        Assert.Equal(heroes[0].AssetId, escrow.BaseId);
        Assert.Equal(heroes[1].AssetId, escrow.SacrificeId);

        // Unknown merge → 404.
        await Assert.ThrowsAsync<ArkadeHeroesApiException>(() => alice.Merge.EscrowAsync("does-not-exist"));
    }
    ...
    public async Task Merge_AbandonedDeposit_RefundReturnsFee_ThenNothingToRefund()
    {
        ...
        await alice.Dev.RefundMergeAsync(new { MergeId = commit.MergeId });
        ...
        await Assert.ThrowsAsync<ArkadeHeroesApiException>(
            () => alice.Dev.RefundMergeAsync(new { MergeId = commit.MergeId }));
    }
}
